...
import { GreaterThan } from '../../common/decorators';

describe('GreaterThan Decorator', () => {
  ...
  it('should create a Product if price is greater than 0', () => {
    const product = new Product(10);
    expect(product.price).to.equal(10);
  });

  it('should throw an error if price is equal to 0', () => {
    expect(() => new Product(0)).to.throw(Error, /must be greater than 0/);
  });

  it('should throw an error if price is less than 0', () => {
    expect(() => new Product(-5)).to.throw(Error, /must be greater than 0/);
  });
  ...
  it('should throw an error when updating price to an invalid value', () => {
    ...
    expect(() => {
      product.price = 0;
    }).to.throw(Error, /must be greater than 0/);
  });
});
